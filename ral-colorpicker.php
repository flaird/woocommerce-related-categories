...
function ralcolorpicker_scripts() {
    ...
    wp_enqueue_style( 'ral-colorpicker', plugins_url( 'css/ral-colorpicker.css', __FILE__ ), array() );
    wp_enqueue_style( 'select2', plugins_url( 'css/select2.min.css', __FILE__ ), array() );
    ...
    if ( ! wp_script_is( 'select2', 'enqueued' )) {
        //Enqueue
        wp_enqueue_script( 'select2', plugins_url( 'js/select2.min.js', __FILE__ ), array('jquery') );
    }
    wp_enqueue_script( 'ral-colorpicker', plugins_url( 'js/ral-colorpicker.js', __FILE__ ), array('jquery') );
}
...
function ralcolorpicker_shortcode( $atts ) {
    ...
    $output = '<select id="ralcp-select" class="ralcp-select">';
    foreach($ralclassic as $optgroup) {
        if($atts['headers'] == 1) { $output .= '<optgroup label="'.$optgroup['name'].'">'; }
        foreach($optgroup['colors'] as $color) {
            $output .= '<option value="'.$color['name'].'" class="ralcp-'. substr($color['hex'], -6).'" style="background-color:'.$color['hex'].'" data-color="'.$color['hex'].'">'.$color['name'].'</option>';
        }
